...
def _build_fg_neg_from_pos_prostate(text_pos: torch.Tensor) -> torch.Tensor:
    """Build prostate-specific foreground negative tokens from positive class tokens.

    Expected positive order (prostate 3-class):
      [BG, PZ, TZ]  => returns [PZneg, TZneg]  (excludes BG)

    With two foreground classes, each neg token is simply the other foreground token:
      PZneg = TZ
      TZneg = PZ

    Args:
      text_pos: (3,512) CLIP text embeddings.

    Returns:
      (2,512) normalized tensor on the same device/dtype.
    """
    if text_pos is None:
        return None
    if (not torch.is_tensor(text_pos)) or text_pos.dim() != 2 or text_pos.shape[1] != 512 or text_pos.shape[0] != 3:
        raise ValueError(
            f"[prostate] text_pos must be a tensor of shape (3,512) as [BG,PZ,TZ]; "
            f"got {type(text_pos)} {getattr(text_pos, 'shape', None)}"
        )

    pz = text_pos[1]
    tz = text_pos[2]

    # leave-one-out mean over foreground (N=2) => swap
    pzneg = tz
    tzneg = pz

    neg = torch.stack([pzneg, tzneg], dim=0)  # (2,512)
                            ...
                            with torch.no_grad():
                                if isinstance(embeddings, dict):
                                    ...
                                    if embeddings.get("neg_mode", "from_pos") == "from_pos":
                                        try:
                                            neg_tok = _build_fg_neg_from_pos_prostate(new_text)
    ...
    def forward(
        ...
        *,
        ...
        if use_tri_cfg:
            ...
            if text_neg is None:
                neg_tok = _build_fg_neg_from_pos_prostate(pos_tok)  # (3,512), excludes BG
                neg_mode = "from_pos"
            else:
                neg_tok = text_neg
                neg_mode = "explicit"
