...
# ---- Data / Experiment ----
parser.add_argument("--root_path", type=str, default="/workspace/dataset/MSCMRSEG/mscmrseg_split1", help="dataset root")
...
parser.add_argument("--labeled_num", type=int, default=7, help="labeled patients")
...
parser.add_argument("--phys_guidance", type=int, default=1,help="Enable edge-aware anisotropic heat conduction during rectifier DDIM sampling (1=on, 0=off)")
...
def patients_to_slices(dataset, patiens_num):
    ...
    if "ACDC" in dataset or "acdc" in dataset_l:
        ref_dict = {
            "1": 32,     # 1%
            "3": 68,     # 5%
            "7": 136,    # 10%
    ...
    elif "task05" in dataset_l:
        assert args.num_classes == 3, "Task05 only has 3 classes"
        if "split1" in dataset_l:
            ref_dict = {"2": 35}
        elif "split2" in dataset_l:
            ref_dict = {"2": 40}
    elif "mscmrseg19" in dataset_l or "mscmrseg" in dataset_l:
        if "split1" in dataset_l:
            ref_dict = {"7": 110}
        elif "split2" in dataset_l:
            ref_dict = {"7": 103}
    ...
    if ref_dict is None:
        raise NotImplementedError(f"Unknown dataset split in root_path: {dataset}")
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for sampled_batch in trainloader:
            ...
            #   The extra complementary rectifier term is removed.
            # -----------------------------
            ...
            if iter_num % 100 == 0 and iter_num> 35000:
                model.eval()
                metric_list = 0.0
...
if __name__ == "__main__":
    ...
    snapshot_path = "/workspace/DiffRect-main/logs/{}_{}_{}_labeled/{}".format(
        args.exp,
        dataset_name,
        args.labeled_num,
        args.model,
