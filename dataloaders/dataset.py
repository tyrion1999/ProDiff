...
class BaseDataSets(Dataset):
    def __init__(
        ...
        elif self.split == "test_change":
            with open(self._base_dir + "/test_change.list", "r") as f:
                self.sample_list = f.readlines()
            self.sample_list = [item.replace("\n", "") for item in self.sample_list]
...
class WeakStrongAugment_Ours(object):
    """
        ...
        注意：
        weak 和 strong 共享几何增强，比如翻转、旋转；
        strong 额外多了颜色增强和高斯模糊。
        """
    ...
    def __init__(self, output_size, args=None, split='train'):
        ...
        self.transform_list_train = {
            'c_random_flip': {'lr': True, 'ud': True},  # 随机左右翻转、上下翻转
            'c_random_rotate': {'range': [0, 359]}, # 随机旋转，默认角度范围 [0, 359]
            'c_random_image_enhance': {'methods': ['contrast', 'sharpness', 'brightness']},  # 随机颜色增强，只作用于 image_strong
            'c_random_gaussian_blur': {'apply': True}, # 随机高斯模糊，只作用于 image_strong
            'c_tonumpy': None, # 把 PIL Image 转成 numpy array
            'c_normalize': {'mean': None, 'std': None}, } # 归一化
        # 测试阶段不做随机增强，只转 numpy 和归一化
    ...
    def get_transform(transform_list):
        """
        把增强配置字典转换成 transforms.Compose。
        例如： 'c_random_flip': {'lr': True, 'ud': True}
        会变成：c_random_flip(lr=True, ud=True)
        注意：
        这里用 eval(key)，要求 key 对应的类名必须已经在当前文件中定义。
        """
    ...
    def __call__(self, sample):
        """
        每次 DataLoader 取一个样本时，会调用这里。
        输入 sample:
        {    "image": 原图 numpy, shape [H, W]
            "label": 标签 numpy, shape [H, W]
        }
        输出 sample_new:
        { "image":        [1, H, W]
            "image_weak":   [1, H, W]
...
class TwoStreamBatchSampler(Sampler):
    ...
    作用：
    每个 batch 同时包含两类样本：
    1. primary_indices   ：有标签数据
    2. secondary_indices ：标签数据
    在你的代码里：
    ...
    labeled_bs = 3
    那么：
    primary_batch_size = 3      # 每个 batch 取 3 个有标签样本
    secondary_batch_size = 5    # 每个 batch 取 5 个无标签样本

    一个 epoch 的长度由 primary_indices 决定。
    secondary_indices 如果不够，会反复随机打乱、循环使用。
    """
    ...
    def __init__(self, primary_indices, secondary_indices, batch_size, secondary_batch_size):
        ...
        # 每个 batch 中主样本数量，也就是 args.labeled_bs
        self.primary_batch_size = batch_size - secondary_batch_size

        # 保证主样本数量至少够组成一个 primary batch
        assert len(self.primary_indices) >= self.primary_batch_size > 0
        # 保证副样本数量至少够组成一个 secondary batch
        assert len(self.secondary_indices) >= self.secondary_batch_size > 0

    def __iter__(self):
        # 主样本：每个 epoch 随机打乱一次，只遍历一轮
        primary_iter = iterate_once(self.primary_indices)
        # 副样本：无限循环随机打乱，因为无标签样本要一直补足每个 batch
        secondary_iter = iterate_eternally(self.secondary_indices)
        # 每次从 primary_iter 取 primary_batch_size 个，每次从 secondary_iter 取 secondary_batch_size 个
        # 然后拼成一个完整 batch
        return (
    ...
    def __len__(self):
        # 一个 epoch 有多少个 batch，由 primary_indices 决定。
